...
#define WRITECHAR(a) \
	...
	bool ScpStream::readWord(std::string& sWord) {
		...
		do {
			iChar = readChar();
			++iIndex;
		} while (((iChar >= (int) 'a') && (iChar <= (int) 'z')) || ((iChar >= (int) 'A') && (iChar <= (int) 'Z')) || ((iChar >= (int) '0') && (iChar <= (int) '9')) || (iChar == (int) '_') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?'));
		if (iChar >= 0) goBack();
		--iIndex;
		if (iIndex == 0) sWord = "";
...
//==        if (iEndPortion < _iWriteCursor || _iSize > _iWriteCursor) _iWriteCursor += iBytesToAdd;
	...
	bool readWord(std::istream& stream, std::string& sWord) {
		...
		} while (((iChar >= (int) 'a') && (iChar <= (int) 'z')) || ((iChar >= (int) 'A') && (iChar <= (int) 'Z')) || ((iChar >= (int) '0') && (iChar <= (int) '9')) || (iChar == (int) '_') || (iChar == (unsigned char) '?') || (iChar == (int) '\'') || (iChar == (int) '^') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?') || (iChar == (unsigned char) '?'));
		if (iChar >= 0) goBack(stream);
		iIndex--;
		tcReadWord[iIndex] = '\0';
